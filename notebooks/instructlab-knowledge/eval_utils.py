...
class DocLoader(BaseLoader):
    def __init__(self, file_path: Path | list[Path]) -> None:
        ...
        for source in self._file_paths:
            if source.suffix == ".md":
                with open(source, "r", encoding="utf-8") as f:
                    ...
                    yield LCDocument(page_content=markdown_content)
...
def load_docs(db: Milvus, document_directory: str):
    file_paths = [path for path in Path(document_directory).rglob("*") if path.suffix in [".md", ".pdf"]]
    loader = DocLoader(file_path=file_paths)
